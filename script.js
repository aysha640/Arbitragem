...
async function salvarNoFirebase() {
    if (window.database && window.dbRef && window.dbSet) {
        try {
            const clientesRef = window.dbRef(window.database, 'aysha/clientes');
            await window.dbSet(clientesRef, {
                dados: clientes,
...
async function carregarDadosFirebase() {
    if (window.database && window.dbRef && window.dbGet) {
        try {
            const clientesRef = window.dbRef(window.database, 'aysha/clientes');
            const snapshot = await window.dbGet(clientesRef);
            
            if (snapshot.exists()) {
                const dadosFirebase = snapshot.val();
                if (dadosFirebase.dados) {
                    clientes = dadosFirebase.dados;
                    localStorage.setItem('aysha_clientes', JSON.stringify(clientes));
                    atualizarInterface();
                    console.log('✅ Dados carregados do Firebase');
                }
            } else {
                console.log('ℹ️ Nenhum dado no Firebase ainda');
                carregarDados(); // Carrega do LocalStorage
            }
            
            // Sincronização em tempo real
            if (window.dbOnValue) {
                window.dbOnValue(clientesRef, (snapshot) => {
                    if (snapshot.exists()) {
                        const dadosFirebase = snapshot.val();
                        if (dadosFirebase.dados) {
                            clientes = dadosFirebase.dados;
...
function mostrarAdicionarDividaCliente(event) {
    if (event) {
...
// Modal - Pagamento
function mostrarPagamentoCliente(event) {
    if (event) {
        event.stopPropagation();
    }
    const cliente = clientes[clienteSelecionado];
